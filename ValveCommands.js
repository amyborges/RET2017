...
function wrap_data_for_Arduino(A,B,C,D) {
  //var valve_to_control = (document.getElementById("ValveNumberSelector").value);
    ...
    var open_state_parameter = B;
    var closed_state_parameter = C;
    var physical_state_parameter = D;

    if (physical_state_parameter)
    {
        var PWMval  = open_state_parameter;
        console.log("Going to the open state : " + PWMval);
    }
    else
    {
   ...
   // PWMval = valve_uL_to_PWM(uL_table,uL_precision,PWMval);
    ...
    console.log("PWM : " + PWMval);
    var PWMval_padded = zeroFill(PWMval,4);
    console.log("PWM Padded : " + PWMval_padded);
...
function sendCommand(A,B,C,D) {
    //var socket = io('http://localhost:3001');
    var command = wrap_data_for_Arduino(A,B,C,D);
